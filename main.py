...
def preprocess_userinput(user_input):
    ...
    if os.path.exists(path):  # the user has provided a file path with a set of texts
        try:
            # Split text file with the delimiter, erase first delimiter
            list_texts = re.split(
        ...
        only_query_id = input("Write the ID of the query provided:\n")
...
def execute_IRsystem_prompt(corpus_text, query_text, only_query_id):
    ...
    irmodel_choice = input(
        "Please, choose an information retrieval model by entering the id of the model:\n")

    ir = create_ir_system(int(irmodel_choice), corpus_text, query_text)

    irevaluator_choice = input(
        "Do you want to execute the performance evaluation of the IR system selected (YES/NO)? \n")

    if((irevaluator_choice == "YES") | (irevaluator_choice == "yes")):
        relevances_input = input(
            "Write the directory path with the document relevances:\n")
    ...
    continue_choice = input(
        "Do you want to execute another IR model (YES/NO)? \n")
    ...
    if((continue_choice == "YES") | (continue_choice == "yes")):
        ...
        ir_evaluator.IREvaluator(
            relevances, ir.ranking_query, False, only_query_id)
    return ir
...
def execute_Rocchio_prompt(query_text, corpus_text, ir, only_query_id):
    ...
    if((rocchio_choice == "YES") | (rocchio_choice == "yes")):
        ...
        pos = 0
        while pos < user_improvement:
            answer = input("Is relevant the document ID " +
                               str(rankings[pos][0]) + " (Y/N)?")
        ...
        while ((answer == 'y') or (answer == 'Y')):
            ...
            answer = input(
                "Do you want to execute again the rocchio optimization algorithm (Y/N)?")
    return
...
if __name__ == '__main__':
   ...
   query_input = input(
      "Write a query or enter a document path with a set of queries:\n")
